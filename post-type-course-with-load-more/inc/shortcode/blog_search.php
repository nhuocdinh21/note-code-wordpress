<?php
// [blog_search]
function shortcode_latest_from_blog_search($atts, $content = null, $tag = '' ) {
  ...
  if($visibility == 'hidden') return;
	...
	if ( !empty( $ids ) ) {
		...
		$args = array(
            ...
            'post_type' => array(
                'post',
                'khoa-hoc', // Include for its tag archive listing.
            ),
			'numberposts' => -1,
			'orderby' => 'post__in',
			'posts_per_page' => 9999,
...
while ( $recentPosts->have_posts() ) : $recentPosts->the_post();
		...
		?><div class="col <?php echo implode(' ', $col_class); ?>" <?php echo $animate;?>>
          ...
          <?php } ?>
					...
					<?php if((!has_post_thumbnail() && $show_date !== 'false') || $show_date == 'text') {?><div class="post-meta is-small op-8"><span><?php echo __('Date Submitted','custom'); ?>: </span><?php echo get_the_date(); ?></div><?php } ?>
...
add_shortcode("blog_search", "shortcode_latest_from_blog_search");
